...
DEMAND_LETTER_DISCLAIMER = (
    "IMPORTANT — DRAFT ONLY / NOT LEGAL ADVICE\n"
    "This document is a consumer-editable draft generated by Mishara (Nisaba LLC). "
    "It is not legal advice, not an attorney work product, and not a filing. "
    "Review, edit, and decide for yourself (or with your own counsel) before sending. "
    "Sending this letter does not create an attorney–client relationship with Nisaba LLC or Mishara."
)


def _with_demand_disclaimer(letter: str) -> str:
    """Ensure the generated demand letter body itself carries draft/not-legal-advice language."""
    text = (letter or "").strip()
    marker = "IMPORTANT — DRAFT ONLY / NOT LEGAL ADVICE"
    if marker in text:
        return text + ("\n" if not text.endswith("\n") else "")
    return f"{DEMAND_LETTER_DISCLAIMER}\n\n{text}\n"
...
def generate_demand_letter(
    ...
    system = (
        ...
        "Do NOT claim to be from an attorney. The letter body will be prefixed with a draft/not-legal-advice "
        "banner by the system — write only the letter content."
    )
    user = (
        f"Platform: {platform}\nDomain: {velaru_domain}\nClassification: {classification}\n"
    ...
    if result:
        return _with_demand_disclaimer(result)
...
[YOUR CONTACT]
"""
    return _with_demand_disclaimer(body)
...
def stripe_session_authorizes_unlock(session: dict, token: str, row: sqlite3.Row) -> bool:
    ...
    if stored and sid and stored != sid:
        return False
    expected_product = row["product_id"]
    if meta.get("product_id") and meta.get("product_id") != expected_product:
        return False
    expected_receipt = row["receipt_hash"]
    if meta.get("receipt_hash") and meta.get("receipt_hash") != expected_receipt:
        return False
    return True
