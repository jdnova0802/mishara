...
| 13 | `--dim 13` | [`borrowed_cred_terms.txt`](./borrowed_cred_terms.txt) + no `/dtcc` routes on Gate/Mishara. Velaru: source floor in Velaru CI; Mishara job `velaru-surface` live-probes `velaru.xyz` for **404** on `/dtcc` paths (**no token**). Prove-fail: `--prove-fail-velaru-live` (unreachable + 308 must fail loud). |
...
python scripts/scorecard_gates/run_gates.py --prove-fail-velaru-live
```
