...
| Science | Gate law |
|---|---|
| **PRI** (pre-irreversibility inhibition) | `execute` iff LIVE ∧ justified; else DENY; no bypass; CHARGE-only re-open |
| **Fail-closed** | uncertainty ⇒ DENY; soft-yes under panic is the failure mode |
| **Thermo intervention** (no perfect `do(·)`) | measure → control → stranger-verifiable receipt; silent free yes illegal |
...
Nuclear football: **state only**. We may someday be a tiny component under them — never the sovereign.
