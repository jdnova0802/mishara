# Outbound sent tally

**Target:** **10 touches** Monday (10/10 cut — Palomar restored via Brandon Loyd).  
**Bump days** count toward the day’s total (short nudges = touches).  
**Rule:** log the same day you send. If the day’s count ≠ planned on a send day, write **GAP** in Notes — don’t discover it two days later.

## How to log

One row per calendar day you touch desks.  
`Count` = emails / form submits / bumps actually sent that day (not drafts sitting in the repo).

| Date | Day type | Plan A | Plan B | Bumps | **Count** | Targets (short) | Notes |
|------|----------|--------|--------|-------|-----------|-----------------|-------|
| 2026-09-23 | Send (Tue) | 4 | 0 | 0 | **4** | Buckle, SteadPoint, 1970, Everspan | **GAP** — thought cadence was 10; delivered Plan A only |
| 2026-09-26 | Bump (Fri) | 0 | 0 | _TBD_ | _TBD_ | same four | Fill when sent |
| 2026-09-29 | Send (Mon) | 4 | 6 | 0 | **planned 10** | Plan A 4 + Plan B 6 (Palomar = Loyd) | SEND — PASTE_MONDAY_DILIGENCE_EMAILS.md 10/10 cut |

## Running check (update weekly)

| Week of | Send-day target | Actual send-day total | Gap? |
|---------|-----------------|----------------------|------|
| 2026-09-22 | 10 (Tue) | 4 | YES (−6) |
| 2026-09-29 | 10 (Mon) | _TBD_ | |

## Cadence reminder

| Format | Pace | Example |
|--------|------|---------|
| **Plan A** | 2–4 / send day | Named desk + verified cite (docket / Artemis with number) |
| **Plan B** | rest to hit 10 | One public pain line + REVIEW CTA — no deep dig |
| **Bumps** | Fri after Tue wave | Short nudge only — still log count |

Draft sources:
- Canonical Monday paste → `PASTE_MONDAY_DILIGENCE_EMAILS.md` (10/10 cut)
- Plan A backup four → `BACKUP_WAVE_72H_DILIGENCE.md`
- Plan B six → `PLAN_B_FAST_6.md`

## Do not reuse

Already burned (do not recount as “new”):  
Buckle · SteadPoint · 1970 · Everspan · Porch · Incline · Clear Blue · Gallagher · (once sent) Sutton · MS Transverse · Aon Re · Palomar · Fortegra · Great Divide.
