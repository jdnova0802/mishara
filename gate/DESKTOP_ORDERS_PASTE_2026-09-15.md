...
Do:
- Add local HTTP stranger-verify for receipts (GET by receipt_id)
- Add FedNow/RTP-shaped bank-send DENY fixtures (not real rails)
- Keep non-pay consequential actus (prod_mutate etc.)
- README honesty: not competing Fidacy/IntentFence or Visa TAP head-on;
  own digest-bound DENY + stranger receipt + bank-push-adjacent / non-pay
Prove: bank-send-shaped DENY; stranger can fetch receipt URL; all prior proves still OK.
...
Do NOT start S2/S4/S5 or S6–S8 until Orders 1–2 are green.

=== ORDER 3 — S7 EDGAR Disclose Seal (weld-shape) ===
Files: gate/sims/edgar_disclose_seal.py, prove_edgar_disclose_seal.py
Pastes: gate/S_TIER_PLUS_THREE_2026-09-15.md (S7)
Do: extend submit mouth toward weld shape (grounded claims → seal → submit);
HTTP stranger receipt; still lab — no real EDGAR.
Prove: hallucinated quantity DENY; no-seal submit DENY; grounded ALLOW + receipt.

=== ORDER 4 — S6 Deed Record Gate (weld-shape) ===
Files: gate/sims/deed_record_gate.py, prove_deed_record_gate.py
Pastes: gate/S_TIER_PLUS_THREE_2026-09-15.md (S6)
Do: extend record-accept mouth; HTTP stranger receipt; still lab — no real county.
Prove: inject_suspect DENY; owner_lock without unlock DENY; LIVE ALLOW + receipt.
Not title insurance. Record mouth only.

=== ORDER 5 — S8 RON Attest Refuse (weld-shape) ===
Files: gate/sims/ron_attest_refuse.py, prove_ron_attest_refuse.py
Pastes: gate/S_TIER_PLUS_THREE_2026-09-15.md (S8)
Do: extend refuse mouth + stranger receipt; still lab — no real notary commission.
Prove: synthetic_suspect REFUSE; unknown REFUSE; live ALLOW + receipt.
Not a notary marketplace. Refuse mouth only.
...
=== HARD TRAPS ===
- Do not beat Visa at checkout. Adjacent only: non-pay actus + bank push + stranger receipt.
- Do not build title insurance / Workiva / notary marketplace.
- Do not invent greenfield grants theater where Fidacy/Visa already own recognition.

=== OUTBOUND (human, not more sim scroll) ===
After P0 green: note EFSP/AmLaw-shaped outbound for S3; bank agent-pay adjacent outbound for S1.
Still lab fixtures only unless a real weld partner exists.

Detailed paste blocks: gate/DESKTOP_PASTES_S_TIER_2026-09-15.md
S6–S8 paste blocks: gate/S_TIER_PLUS_THREE_2026-09-15.md
...
> Extend gate/sims P0: S1 HTTP verify + bank-send-shaped DENY, then S3 EFSP block-transmit. Then weld-shape S7→S6→S8. Defer S2/S4/S5. Fail-closed + stranger receipts. `their_production: false`. Don’t rebuild cores cloud already proved.
