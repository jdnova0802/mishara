...
class OperatorInvoiceTests(unittest.TestCase):
    ...
    def test_positioning_manifest_and_page(self):
        ...
        self.assertEqual(page.status_code, 200)
        body = page.get_data(as_text=True)
        self.assertIn("Exclusive door", body)
        self.assertIn("CHARGE port", body)
        self.assertIn("Distribution stack", body)

        spec = self.client.get("/.well-known/positioning.json")
        self.assertEqual(spec.status_code, 200)
        data = spec.get_json()
        self.assertEqual(data["spec"], "gate-positioning-v1")
        self.assertIn("public_face", data)
        ...
        pf = self.client.get("/.well-known/public-face.json")
        self.assertEqual(pf.status_code, 200)
        pf_data = pf.get_json()
        self.assertEqual(pf_data["spec"], "gate-public-face-v1")
        self.assertEqual(len(pf_data["pillars"]), 3)
    ...
    def test_focus_page_core_icps(self):
        ...
        self.assertEqual(r.status_code, 200)
        body = r.get_data(as_text=True)
        self.assertIn("Three faces. One hull.", body)
...
class SettlementEngineTests(unittest.TestCase):
    ...
    def test_invention_wave_bayesian_costliness_fulfillment_variety_closure_weld(self):
        ...
        gate = self.client.get("/.well-known/gate.json").get_json()
        self.assertIn("public_face", gate)
        self.assertIn("door", gate["public_face"])
        self.assertIn("charge", gate["public_face"])
        self.assertIn("distribution", gate["public_face"])
        self.assertIn("catalog", gate)
        self.assertIn("discovery", gate["catalog"])
        self.assertIn("costliness", gate["catalog"]["discovery"])
        self.assertIn("performative", gate["catalog"]["discovery"])
        self.assertIn("moat", gate["catalog"]["discovery"])
        self.assertIn("clinamen", gate["catalog"]["discovery"])
        self.assertIn("apophatic", gate["catalog"]["discovery"])
        self.assertIn("agential_cut", gate["catalog"]["discovery"])
        self.assertIn("modes", gate["catalog"]["discovery"])
        self.assertIn("xenohardware", gate["catalog"]["discovery"])
        self.assertIn("mouth_isa", gate["catalog"]["discovery"])
        self.assertIn("firmware_fuse", gate["catalog"]["discovery"])
        self.assertIn("no_software", gate["catalog"]["discovery"])
        self.assertIn("landauer", gate["catalog"]["discovery"])
        self.assertIn("nmi_halt", gate["catalog"]["discovery"])
        self.assertIn("holographic", gate["catalog"]["discovery"])
        self.assertIn("secure_boot", gate["catalog"]["discovery"])
        self.assertIn("interlock", gate["catalog"]["discovery"])
        self.assertIn("exochronology", gate["catalog"]["discovery"])
        self.assertIn("unlanguage", gate["catalog"]["discovery"])
        self.assertIn("nonabelian", gate["catalog"]["discovery"])
