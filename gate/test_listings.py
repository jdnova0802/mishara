...
class BindRoomFlaskTests(unittest.TestCase):
    ...
    def test_diligence_surfaces(self):
        ...
        self.assertIn(b"Free 72-hour review", page.data)
        self.assertIn(b"Email REVIEW", page.data)
        self.assertNotIn(b"Email DEPOSIT", page.data)
        offer = self.client.get("/diligence/offer.json")
        self.assertEqual(offer.status_code, 200)
        data = offer.get_json()
        self.assertEqual(data["spec"], "gate-ops-review-v2")
        self.assertEqual(data["price"]["deposit"], "$2,500")
        self.assertIn("after the free review", data["price"]["deposit_due"])
        self.assertEqual(data["ask"], "Reply REVIEW — no invoice until after the free review.")
        self.assertIn("/diligence", data["urls"]["page"])
        one = self.client.get("/diligence/one-pager.txt")
        self.assertEqual(one.status_code, 200)
        self.assertIn(b"Reply REVIEW", one.data)
        self.assertNotIn(b"Reply DEPOSIT", one.data)
