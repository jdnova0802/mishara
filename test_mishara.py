...
class MisharaProductsTest(unittest.TestCase):
    ...
    def test_paid_query_param_does_not_unlock_without_stripe(self):
        ...
        self.assertNotIn("Generate deliverable", html)
        row = app.get_unlock(token)
        self.assertEqual(row["status"], "checkout_open")
        # Fulfillment must still be gated (unpaid POST must not return a letter)
        blocked = self.c.post(
            f"/unlock/{token}",
    ...
    def test_stripe_session_id_unlocks_only_when_paid(self):
        ...
        with mock.patch.object(app, "fetch_stripe_checkout_session", return_value=unpaid):
            page = self.c.get(f"/unlock/{token}?session_id=cs_test_real_session_001")
            self.assertEqual(page.status_code, 200)
            self.assertEqual(app.get_unlock(token)["status"], "checkout_open")
